"""Serialise benchmark outputs (semantic entropy, class probabilities, judge
scores, prompts, step histories) produced by ``src.process_data`` into a
JSON file. Corresponds to the aggregation step of the step-wise evaluation
pipeline described in the paper (Section 5).

Usage (positional argv, preserved for backward compatibility with existing
job scripts):

    python export_data.py <model> <output_json> <factuality: chateval|llmjudge>
                          <entailment: gpt4|deberta> <llm_judge: true|false>
                          <temperature> <num_problems> <hs_toggle: hs|nohs>
                          <starting_problem>
"""
