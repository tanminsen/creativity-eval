"""GPT-4o runner for the step-wise MacGyver benchmark (paper §5).

Populates module-level ``fullscale_*`` arrays with per-problem / per-step
candidate generations, token logprobs, semantic clusters and retained
greedy-decoded steps. Intended to be imported by ``src.process_data``
(which reduces these arrays into Semantic Entropy + judge outputs).
"""
...
# NOTE: this runner iterates over range(1) (single problem), unlike the
# other per-model runners which honour sys.argv[7] (num_problems). Left
# unchanged — see repo polish flag #14.
for i in range(1):
  prev_steps = []
