"""HuggingFace Transformers generation wrappers for Llama, Vicuna and
Mistral-family models. Each ``gen_prob*`` function samples ``num``
candidate continuations at the configured temperature / top-p and returns
``(responses, tokens, logprobs, hidden_states)`` for downstream Semantic
Entropy computation (paper §5, sampling n=10 at T=1.0, top_p=0.9).

Model path is selected via ``sys.argv[1]`` (llama / llama3.3 / vicuna /
mistral-nemo / …). Temperature comes from ``sys.argv[6]`` and hidden-state
toggling from ``sys.argv[8]``. 70B-class models are loaded in 8-bit via
bitsandbytes.
"""
...
# Default HuggingFace cache location for the NSCC environment used in the
# paper experiments. Only applied when the environment variable is unset,
# so callers can override by exporting HF_HOME / TRANSFORMERS_CACHE before
# running.
os.environ.setdefault("HF_HOME", "~/scratch/macgyversemanticprobing/.cache/")
os.environ.setdefault("TRANSFORMERS_CACHE", "~/scratch/macgyversemanticprobing/.cache/")
...
def convert_openai_to_llama_prompt(ls):
    """Flatten an OpenAI-style chat message list into a single Llama-3
    prompt string with the native header / eot tokens."""
    pmpt = '<|begin_of_text|>'
    for msg in ls:
        if msg['role'] == 'user':
...
def gen_prob(problem ,prompt, num=1, verify=False, include_eg = True):
    """Sample ``num`` candidate continuations from the loaded Llama model,
    returning ``(responses, tokens, logprobs, hidden_states)`` per
    paper §5. Uses nucleus sampling (top_p=0.9) at the configured
    temperature, with optional re-generation if the response contains
    more than two ``Step`` headers. Hidden states are captured only when
    ``toggle_hs`` is active; otherwise the last list entry is empty.
    """
    responses = []
    tokenlist = []
    problist = []
...
def gen_prob_mistral(problem ,prompt, num=1, verify=False, include_eg = True):
    """Mistral-family variant of :func:`gen_prob`. Applies the Mistral
    chat template and strips the trailing ``[/INST]`` marker from the
    decoded response."""
    responses = []
    tokenlist = []
    problist = []
...
def gen_prob_vicuna(problem ,prompt, num=1, verify=False, include_eg = True):
    """Vicuna variant of :func:`gen_prob`. Uses the flat Vicuna prompt
    format rather than Llama-3 header tokens."""
    responses = []
    tokenlist = []
    problist = []
    hiddenstates = []
