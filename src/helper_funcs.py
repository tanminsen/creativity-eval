"""Utility library for the creativity evaluation pipeline.

Contains: sequence log-probability math, class-probability reduction,
Semantic Entropy variants (paper Eqs. 3–5), Rao-Blackwellised estimation
(Appendix C.1), chat-message construction, and scoring / metric helpers
used by the benchmark runners and ``src.process_data``.
"""
...
# Example usage (left as documentation; disabled so it does not run at import).
...
# Example usage (left as documentation; disabled so it does not run at import).
# probabilities = [0.9, 0.8, 0.85, 0.95]
...
def calc_sequence_probability_LOGPROB(probabilities, return_logprob = False):
    """Sum of token log-probabilities for a sequence, returned either as a
    raw log-probability or exponentiated. Numerically clamps outliers
    below -1000 to avoid underflow from padding tokens.
    """
    # print(probabilities)
    logprob = 0
...
def calculate_SE_simple(class_probs):
  """Shannon entropy over unnormalised semantic-class probabilities
  (paper Eq. 3)."""
  SE = 0
...
def calculate_SE_complex(class_probs):
  """Rao-Blackwellised estimator of Semantic Entropy: renormalises the
  class probabilities into a categorical before taking entropy
  (paper Appendix C.1, Eq. 4)."""
  SE = 0
  sum_classprobs = 0
...
def gen_factuality_score_chateval_likert(question, ans, criterialist):
    score = 0
    scores = get_factuality_chateval_likert(criterialist, question, ans)
    feasibility = True
    efficiency = True
    scorearray = []
...
def gen_chat_object(prompt, problem, include_eg = True): # for LLAMA and vicuna
    """Build the MacGyver prompt string fed into Llama / Vicuna generation,
    with an optional one-shot example (paper §5)."""
    example_problem = """Problem: You need to remove the pit from an avocado for your salad, but your utility knife is blunt. In your drawer, you find a stainless steel egg whisk, a wooden rolling pin, a wine bottle filled with Merlot, a heavy-duty garlic crusher, small plastic spoons, and a fork with prongs bent slightly inwards. Unfortunately, the only spoon available isn't sufficient for the task. How should you proceed to remove the pit?
    Existing steps, if any: 
    Response: """
    example_step = "Step 1: Take the fork with the bent prongs and pierce the avocado in an outline around the pit."
...
def gen_chat_object_mistral(prompt, problem, include_eg = True): # for Mistral and GPT-4o
    """Build a chat-message list (system / user / assistant roles) for
    Mistral-family and GPT-4o inference, with an optional one-shot
    example (paper §5)."""
    example_problem = """Problem: You need to remove the pit from an avocado for your salad, but your utility knife is blunt. In your drawer, you find a stainless steel egg whisk, a wooden rolling pin, a wine bottle filled with Merlot, a heavy-duty garlic crusher, small plastic spoons, and a fork with prongs bent slightly inwards. Unfortunately, the only spoon available isn't sufficient for the task. How should you proceed to remove the pit?
    Existing steps, if any: 
    Response: """
...
def calc_sequence_probability_LOGPROBN(probabilities, return_logprob = False):
    """Length-normalised variant of :func:`calc_sequence_probability_LOGPROB`:
    averages the token log-probabilities rather than summing, counteracting
    the length bias noted in paper Appendix C.1, Eq. 5.
    """
    # print(probabilities)
    count = 0
    total = 0
