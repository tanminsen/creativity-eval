"""OpenAI client wrappers used across the creativity benchmark.

Provides:

- GPT-4o entailment (``get_entailment``, ``get_entailment_openai``) used
  as an optional alternative to the DeBERTa NLI entailment in
  :mod:`src.dabertaMNLI` (paper Appendix C.3);
- single-agent factuality judges (binary and Likert variants,
  ``get_factuality`` / ``get_factuality_likert``) and the ChatEval
  baseline (``get_factuality_chateval_{binary,likert}``) — both used as
  baselines in paper Table 2;
- ``gen_C``: the bidirectional-entailment greedy semantic-clustering
  algorithm (paper §3.2, Appendix C.3.3);
- ``generate_data_from_GPT``: candidate sampling with logprobs for the
  GPT-4o runner.
"""
...
def get_factuality_chateval_binary(crit, question, ans):
    """ChatEval baseline (Chan et al., 2023) used as a comparison point in
    paper Table 2: two debater agents produce arguments for and against
    the solution fulfilling each criterion, then a judge agent returns a
    binary verdict per criterion.
    """
    rounds = 2
    evals = []
...
def gen_C(x, ls, tokenseq, probsq):
    """Greedy bidirectional-entailment clustering of a set of generations
    into semantic classes (paper §3.2, Appendix C.3.3).

    For each sample, checks bidirectional entailment against the first
    member of each existing class; appends to the first matching class or
    creates a new one. Returns parallel (classes, tokens, probs) lists
    ``(C, T, P)`` with identical nested structure. Uses DeBERTa NLI when
    ``sys.argv[4] == 'deberta'``, otherwise GPT-4o entailment.
    """
    C = [[ls[0]]]
    T = [[tokenseq[0]]]
    P = [[probsq[0]]]
...
def generate_data_from_GPT(problem ,prompt, num=1, verify=False, include_eg = True):
    """Sample ``num`` candidate continuations from GPT-4o for a given
    problem / prompt, carrying per-token logprobs through for downstream
    Semantic Entropy computation (paper §5)."""
    responses = []
    problist = []
    tokenlist = []
    max_tokens = 1024
